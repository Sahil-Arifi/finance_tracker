...
The focused Node test suite covers webhook signatures, exact body verification, replay age, transaction removals, pagination retries, atomic cursor persistence, and client/server write interleavings. Install both root and Functions dependencies before running `npm test`. These tests use generated signing keys and mocked storage; they do not certify a live bank integration. The full repository lint command still reports existing UI issues and an unused backend helper.

The finance document currently stores transaction arrays, which limits scalability. Server sync commits its cursor with the imported rows. Client saves apply changes relative to the last acknowledged snapshot inside a Firestore transaction, preserving unseen server additions and removals. Concurrent edits to the same field resolve in transaction commit order; the interface does not yet display a conflict resolution history. Validate the full flow with Plaid Sandbox before deploying integration changes. AI answers and receipt extraction can be incomplete or incorrect and should be checked against the source records.
